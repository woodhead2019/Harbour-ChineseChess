...
# Harbour 编译器路径
HBMK2="/opt/harbour/bin/linux/gcc/hbmk2"
#HBMK2="/tmp/harbour-linux/bin/hbmk2"
HB_HOST_BIN="/opt/harbour/bin/linux/gcc"
...
# 检查 hbmk2 是否存在 -- 改为更鲁棒的检测：先检查指定路径，若不存在再尝试 PATH
if [ ! -f "$HBMK2" ]; then
    if command -v hbmk2 >/dev/null 2>&1; then
        HBMK2="$(command -v hbmk2)"
        echo "使用 PATH 中的 hbmk2: $HBMK2"
    else
        echo "错误: 找不到 hbmk2: $HBMK2"
        echo "请确认前面的 Harbour 构建步骤是否成功并已安装 /opt/harbour/bin/hbmk2，或修改本文件开头的 HBMK2 变量指向有效的 hbmk2 可执行文件。"
        echo "当前 PATH=$PATH"
        echo "列出 /opt 目录以便诊断："
        ls -l /opt || true
        echo "列出 /opt/harbour（若存在）："
